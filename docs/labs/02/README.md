...
4. При выполнении лабораторной работы придерживайтесь следующей
...
представлены на макете, а также:
...
6. **Создайте веб-сервер**, который будет
...
В каталог `shared` помещайте те ресурсы, которые являются общими для всех 
страниц сайта (например, логотип сайта).
