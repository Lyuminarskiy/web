...
1. Используйте следующие поля:
    ...
    `female`.
    - Поле для вввода имени файла. Установите фильтр на типы файлов, чтобы 
    можно было отправлять только изображения.
...
2. Все поля в группе `Основная информация` сделайте обязательными.
3. Добавьте красную рамку либо красный цвет текста для полей, являющихся 
обязательными, но ещё не заполненными.
4. Для всех текстовых полей добавьте подсказывающий текст.
5. Добавьте декоративную линию для текста выбранного переключателя.
6. Добавьте кнопки `Отправить` и `Сбросить`.
