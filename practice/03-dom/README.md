...
- [5. Размеры, прокрутка и координаты элементов](#5-Размеры-прокрутка-и-координаты-элементов)
  - [5.1. Ссылки](#51-Ссылки)
  - [5.2. Задачи](#52-Задачи)
    - [5.2.1. Найти высоту невидимой части содержимого снизу](#521-Найти-высоту-невидимой-части-содержимого-снизу)
    - [5.2.2. Найти ширину полосы прокрутки](#522-Найти-ширину-полосы-прокрутки)
    - [5.2.3. Поместить элемент в центр страницы](#523-Поместить-элемент-в-центр-страницы)
...
## 5. Размеры, прокрутка и координаты элементов

На картинке ниже представлены все свойства элемента (метрики), относящиеся к его размерам:
...
| Свойство       | Описание                                                                                           |
| -------------- | -------------------------------------------------------------------------------------------------- |
| `offsetParent` | Ссылка на родительский элемент, от которого ведётся позиционирование элемента.                     |
| `offsetLeft`   | Отступ от левого края элемента `offsetParent`.                                                     |
| `offsetTop`    | Отступ от верхнего края элемента `offsetParent`.                                                   |
| `offsetWidth`  | Внешняя ширина элемента (включая рамки).                                                           |
| `offsetHeight` | Внешняя высота элемента (включая рамки).                                                           |
| `clientLeft`   | Отступ от левого края элемента до его содержимого (включая полосу прокрутки).                      |
| `clientTop`    | Отступ от верхнего края элемента до его содержимого (включая полосу прокрутки).                    |
| `clientWidth`  | Ширина видимой части содержимого элемента (вместе с внутренним отступом, но без полосы прокрутки). |
| `clientHeight` | Высота видимой части содержимого элемента (вместе с внутренним отступом, но без полосы прокрутки). |
| `scrollWidth`  | Полная ширина содержимого элемента (вместе с внутренним отступом, но без полосы прокрутки).        |
| `scrollHeight` | Полная высота содержимого элемента (вместе с внутренним отступом, но без полосы прокрутки).        |
| `scrollLeft`   | Ширина невидимой части содержимого элемента слева.                                                 |
| `scrollTop `   | Высота невидимой части содержимого элемента сверху.                                                |

Метрики невидимых элементов равны нулю.

Свойства `scrollLeft` и `scrollTop` можно изменять (будет выполняться прокрутка содержимого элемента). Также для выполнения прокрутки существует несколько встроенных методов:
...
Для получения координат элемента от границ текущей видимой области необходимо воспользоваться функцией `elem.getBoundingClientRect()`, которая возвращает объект со следующими свойствами:
...
С помощью функции `document.elementFromPoint(x, y)` можно получить элемент, который находится на координатах `(x, y)` относительно текущей видимой области.

### 5.1. Ссылки

- [Метод `Window.scrollBy()`](https://developer.mozilla.org/ru/docs/Web/API/Window/scrollBy)
- [Метод `Window.scrollTo()`](https://developer.mozilla.org/ru/docs/Web/API/Window/scrollTo)
- [Метод `Element.scrollIntoView()`](https://developer.mozilla.org/ru/docs/Web/API/Element/scrollIntoView)
- [Метод `Element.getBoundingClientRect()`](https://developer.mozilla.org/ru/docs/Web/API/Element/getBoundingClientRect)

### 5.2. Задачи

#### 5.2.1. Найти высоту невидимой части содержимого снизу

Свойство `elem.scrollTop` содержит высоту невидимой части содержимого элемента сверху.

Задание:
1. написать скрипт, который будет рассчитывать высоту невидимой части содержимого элемента снизу,
2. проверить работоспособность скрипта.
...
#### 5.2.2. Найти ширину полосы прокрутки

Задание:
1. написать скрипт, который будет рассчитывать ширину стандартной полосы прокрутки,
2. проверить работоспособность скрипта.
...
Этот скрипт нужно применять для элементов, у которых есть вертикальная полоса прокрутки (`overflowY`), но нет рамки (`border`) и внутреннего отступа (`padding`).

<hr>
</details>

#### 5.2.3. Поместить элемент в центр страницы
...
Написать скрипт, который поместит элемент с идентификатором `circle` в центр страницы.
...
circle.style.left = Math.round(document.documentElement.clientWidth / 2 - circle.offsetWidth / 2) + 'px';
circle.style.top = Math.round(document.documentElement.clientHeight / 2 - circle.offsetHeight / 2) + 'px';
```
