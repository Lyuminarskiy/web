...
JS можно вставить в любое место HTML при помощи элемента `<script>`. JS помещается между открывающим и закрывающим тегом элемента `<script>`:
...
Действия браузера:
1. начинает разбор HTML, отображает часть до `<script>`,
2. исполняет содержимое `<script>`,
3. продолжает разбор HTML, отображает оставшуюся часть.
...
Как и CSS, JS можно выносить в отдельные файлы. Это позволяет разгрузить HTML и задействовать кеш браузера для JS. Для указания ссылки на внешний файл JS используется атрибут `src` элемента `<script>`.
...
Задание:
1. создайте страницу `index.html`, которая с помощью функции `alert()` выводит на экран сообщение `Я изучаю JavaScript!`,
